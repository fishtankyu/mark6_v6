...
public partial class AdminWindow : Window
{
    ...
    private void LoadUsers()
    {
        ...
        foreach (var p in UserService.GetAll())
            ...
            {
                Name = p.Name, AllowedRacks = new List<string>(p.AllowedRacks),
    ...
    private void BtnSaveUser_Click(object sender, RoutedEventArgs e)
    {
        ...
        { _editingUser.Name = name; _editingUser.AllowedRacks = racks; _editingUser.LastFourNric = nric; }
        else
        { var u = new AdminUserViewModel { Name = name, AllowedRacks = racks, LastFourNric = nric }; _users.Add(u); _editingUser = u; }

        PersistUsersJson();
        UserList.Items.Refresh();
        ActivityLog.AdminAction((isNew ? $"Added user '{name}'" : $"Updated user '{name}'") + (nric != "" ? $" NRIC:{nric}" : ""));
    ...
    private static void EnrolPhoto(string name, string? src, string dir, Action onSuccess)
    {
        ...
        try
        {
            ...
            MessageBox.Show($"'{name}' enrolled.\n\nRestart the kiosk to apply.", "Enrolled", MessageBoxButton.OK, MessageBoxImage.Information);
            onSuccess();
        }
